...
#' @examples
#' priors <- mahalLocallocalPriors(cent.mean = rep(0, 4),
...
mahalLocallocalPriors <- function(cent.mean = NULL,
                                  cent.prec = NULL,
                                  R = NULL,
                                  beta.mean = NULL,
                                  beta.prec = NULL){
  
  if(dim(R)[1] != dim(R)[2] | length(cent.mean) != length(cent.prec)){
    stop("Please make sure that cent.mean, cent.prec, cent.mean and cent.prec contain the same  number of elements.\n
